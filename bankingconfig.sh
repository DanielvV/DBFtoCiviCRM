...
mysql -u root civicrm << RunSomeMySQL
  ...
  update civicrm_bank_plugin_instance set config='{"generate":1,"auto_exec":true,"ignore":[{"field":"purpose","regex": "#^[^8]#","message": "Dit is geen 8... grootboekrekening"}]}' WHERE plugin_class_id=$ignore;
  update civicrm_bank_plugin_instance set config='{"defaults":{"payment_instrument_id":"5"},"rules":[{"from":"MutatieRegels","to":[{"from":"cMutatieListRegel","to":[{"from":"BedragInvoer","to":"amount","type":"amount"},{"from":"TegenrekeningCode","to":"purpose","type":"set"}],"type":"object"}],"type":"object"},{"if":"equalto:GeldUitgegeven","from":"Soort","to":"_tmp","type":"set"},{"from":"amount","to":"_tmp","type":"append:"},{"from":"_tmp","to":"amount","type":"replace:GeldUitgegeven:-"},{"from":"Datum","to":"booking_date","type":"strtotime:Y-m-d\\\\\\\\TH:i:s"},{"from":"Datum","to":"value_date","type":"strtotime:Y-m-d\\\\\\\\TH:i:s"},{"from":"Omschrijving","to":"description","type":"set"},{"from":"Omschrijving","to":"description","type":"regex:#(.*) NONREF\\\\\\\\z#"},{"comment":"extract IBAN","from":"description","to":"_party_IBAN","type":"regex:#([A-Z]{2}[0-9]{2}[A-Z0-9]*)\\\\\\\\b#"},{"comment":"extract BIC","from":"description","to":"_party_BIC","type":"regex:#\\\\\\\\b([A-Z]{6}[A-Z0-9]{2})\\\\\\\\b#"},{"comment":"extract Contact id","from":"description","to":"contact_id","type":"regex:#\\\\\\\\b[0-9]{1}8[0-1]{1}[0-9]{1}0000([0-9]{7})0#"},{"comment":"extract Name","from":"description","to":"name","type":"regex:#\\\\\\\\b[A-Z]{6}[A-Z0-9]{2} (.*)\\\\\\\\z#"},{"comment":"extract Name","from":"name","to":"name","type":"regex:#\\\\\\\\A(?P<name>.*)?\\\\\\\\b[0-9]{1}8[0-1]{1}[0-9]{1}0000([0-9]{7})0\\\\\\\\z#"}]}' WHERE plugin_class_id=$eboekhouden;
